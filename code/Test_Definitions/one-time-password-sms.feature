Feature: CAMARA OTPvalidationAPI, v1.1.0-rc.1
...
Scenario: Validation for sucess send-code scenario
    ...
    And the resource "/one-time-password-sms/v1.1.0-rc.1/send-code"
...
Scenario:  Validation for sucess validate-code scenario
    ...
    And the resource "/one-time-password-sms/v1.1.0-rc.1/validate-code"
    When the HTTP "POST" request is sent
...
Scenario: Missing {{code}} in message request attribute
    ...
    And the resource "/one-time-password-sms/v1v1.1.0-rc.1/send-code"
    When the HTTP "POST" request is sent
    Then the response property "$.status" is 400
    And the response property "$.code" is "INVALID_ARGUMENT"
    And the response property "$.message" contains a user friendly text
